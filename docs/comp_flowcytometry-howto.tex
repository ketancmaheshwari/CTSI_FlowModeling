...
\texttt{ls \#list files and dirs} \\
\texttt{pwd \#print current dir path} \\
\texttt{cd <dirname> \#change to dir <dirname>} \\
\texttt{cd \#change to the home dir } \\
\texttt{cat <filename> \#display contents of file <filename>} \\
\texttt{cp file1 file2 \#copy file1 to file2 } \\
\texttt{mv file1 file2 \#rename file1 to file2 } \\
\texttt{mv file1 <dirname>/ \#move file1 to <dirname> } \\
\texttt{touch anewfile \#create a new file anewfile  } \\
\texttt{rm <filename> \#delete a file named <afilename>  } \\
\texttt{mkdir <adirname> \#create a dir named <adirame>  } \\
\texttt{man rm \#read manual of the command rm } \\
\texttt{grep 'word' afile \#search for 'word' in afile } \\
\texttt{exit \#exit out of the terminal } \\
\texttt{nano afile \#open the nano editor to edit afile } \\
}

%----------------------------------------------------------------
%	Hardware
%----------------------------------------------------------------
\headerbox{Programs}{name=hardware,column=1, span=2, row=1, below=commoncommands}{
